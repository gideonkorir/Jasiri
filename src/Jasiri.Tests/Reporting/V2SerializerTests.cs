...
namespace Jasiri.Tests.Reporting
{
    public class V2SerializerTests
    {
        ...
        public void SerializerReturnsEmptyJsonArrayOnEmptyOrNullList()
        {
            ...
            Assert.Equal(empty, serializer.Serialize(new ISpan[0]));
        }
        ...
        public void SerializerSerializesFullSpanCorrectly()
        {
            ...
            using (span = tracer.BuildSpan("test")
                ...
                .AsChildOf(new SpanContext(345, 2542, 3535, true, true))
                .Start())
            {
            ...
            var serializer = new V2JsonSerializer();
            var jobj = JArray.Parse(serializer.Serialize(new ISpan[] { span }));
            var spanObj = jobj[0];
            ...
            Assert.Equal(SpanKind.CLIENT, spanObj["kind"].Value<string>());
            Assert.Equal(ZipkinUtil.ToUnixMs(zSpan.StartTimeStamp), spanObj["timestamp"].Value<long>());
            Assert.Equal(ZipkinUtil.DurationMs(zSpan), spanObj["duration"].Value<long>());
            Assert.Equal(zSpan.TypedContext.Debug, spanObj["debug"].Value<bool>());
            Assert.Equal(zSpan.TypedContext.Shared, spanObj["shared"].Value<bool>());
